...
filepath = os.path.join(here,'highscore.txt')
fontpath = os.path.join(here,'arcade.ttf')
fontpath_mario = os.path.join(here,'mario.ttf')
    ...
    while run:
        ...
        min_tracking_confidence=0.5) as hands:
            while cap.isOpened():
                ...
                frame = pygame.surfarray.make_surface(image).convert()
                ...
                #Import the logic that captures the hand input
                action = hand_signs.check_for_input_tetris(results,image)

                if len(last_three_actions) == 3:
                    last_three_actions.pop(0)
                    last_three_actions.append(action)
                else:
                    last_three_actions.append(action)

                if len(set(last_three_actions)) == 1 and len(last_three_actions) == 3 :
                    last_three_actions = []
            ...
            cap.release()
            pygame.quit()
            subprocess.call("py ./main_logic.py")
            sys.exit()
